...
- [ ] 角色设定:身份 / 性格 / 说话风格 / 口癖 / 记忆设定 / 行为边界 → 写进 `SYSTEM_PROMPT`
- [ ] DM / 群 / `公共` 三套人设(DM 专用 prompt,复用 `PUBLIC_SYSTEM_PROMPT` 的分模式)
- [ ] `PREFILL_ASSISTANT` 预填开场白锚住角色语气(低成本模型降出戏效果显著)
- [ ] 一致性规则:不自称 AI、保持口吻、拒答超纲元问题、防角色漂移
- [ ] **模型选型(取代破甲)**:若 DeepSeek 审查挡内容 → 把 `API_BASE_URL`/`API_MODEL` 指向更松端点(海外经代理 / 国内更松 / 自建开源),httpx 走 `HTTPS_PROXY`;按新模型重调人设
